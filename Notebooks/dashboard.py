...
def launch_dashboard(match_networks, club_networks, sna_df, match_metadata, node_limit=200):
    """
    Launches the interactive dashboard UI for visualizing toxicity by matchday,
    club, and overall club comparison.

    Parameters:
        match_networks (dict): Mapping of match_id to networkx graphs
        club_networks (dict): Mapping of club name to networkx graphs
        sna_df (pd.DataFrame): DataFrame with toxicity labels
        match_metadata (pd.DataFrame): Metadata about matches
        node_limit (int): Maximum nodes to display in interactive network
    """
